...
switch State
    ...
    case 'Search'
        ...
        %Update object tracking only when not above base aka (0,0)
        if x >= 0.9 || y >= 0.9
        ...
        if obj.Decisions.Mode && norm(obj.States(1:3)-obj.Waypoints(obj.WP,1:3)') < 1e-2
            ...
            if strcmp(obj.SearchType,"Smart")
                ...
                if gp
                    ...
                    % Check state
                    [state, falsePositive] = IdentifyState(obj, obj.States, obj.Decisions.Mode);
                    
                    % Undo changes
                    obj.Decisions.Gridpoints(gp +1) = 1;
                    obj.BatteryLevel = obj.BatteryLevel + obj.BatteryLossRate;
                else
